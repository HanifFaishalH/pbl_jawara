...
class IuranController extends Controller
{
    ...
    public function tarikIuran(Request $request)
    {
        ...
        try {
            ...
            // Kirim notifikasi ke warga yang ditagih
            foreach ($wargas as $warga) {
                if ($warga->user_id) {
                    NotifikasiHelper::create(
                        userId: $warga->user_id,
                        judul: 'Tagihan Baru',
                        pesan: "Anda memiliki tagihan iuran sebesar Rp " . number_format($request->nominal, 0, ',', '.'),
                        tipe: 'info',
                        link: '/iuran'
                    );
                }
    ...
    public function verifikasiPembayaran(Request $request, $id)
    {
        ...
        try {
            ...
            $warga = $pembayaran->tagihan->warga;
            if ($warga && $warga->user_id) {
                $tipeNotif = $request->status_verifikasi === 'Diterima' ? 'success' : 'warning';
                $pesanNotif = $request->status_verifikasi === 'Diterima' 
                    ? "Pembayaran iuran Anda telah diverifikasi dan diterima."
                    : "Pembayaran iuran Anda ditolak. " . ($request->catatan_verifikasi ?? '');
                
                NotifikasiHelper::create(
                    userId: $warga->user_id,
                    judul: 'Status Pembayaran Iuran',
                    pesan: $pesanNotif,
                    tipe: $tipeNotif,
                    link: '/iuran'
                );
